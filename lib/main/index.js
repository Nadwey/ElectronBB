...
class MainBridge {
    constructor() {
        ...
        ipcMain.handle(`electronbb_call`, async (e, funcID, args) => {
            ...
            if (func) return JSON.stringify(this.CreateExportObject(await func(...args)));
        });

        ipcMain.on("electronbb_callSync", (e, funcID, args) => {
            const func = this.functions.get(funcID);
            if (func) e.returnValue = JSON.stringify(this.CreateExportObject(func(...args)));
        });
    }
    ...
    Export(name, object) {
        ...
        this.exports.set(name, JSON.stringify(exportObject));
    }
